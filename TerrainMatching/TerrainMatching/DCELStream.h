    ...
    */
    ...
    static void copyDcelData(const MeshT& source, MeshT2& target )
    {
        ...
        for( unsigned int faceId=0; faceId<numFaces; ++faceId )
        {
            ...
            typename MeshT::EdgeIterator it(face);
            while( it.hasNext() )
            {
...
void DCELStream<MeshT>::write(const MeshT& mesh, std::ostream &stream)
{
    ...
    for( unsigned int edgeId=0; edgeId<numEdges; ++edgeId )
    {
        const typename MeshT::HalfEdge* edge = mesh.getHalfEdge( edgeId );
...
void DCELStream<MeshT>::read_v1(MeshT& mesh, std::istream &stream)
{
    ...
    while( reading && stream.good() )
    {
        ...
        if( str=="v" )
        {
            typename MeshT::Vertex* vertex = mesh.getVertex( mesh.createVertex() );
        ...
        else if( str=="f" )
        {
            ...
            typename MeshT::Face* face = mesh.getFace( fid );
            stream >> face->getData();
        }
        else if( str=="e" )
        {
            ...
            stream >> from >> to;
            typename MeshT::HalfEdge* edge = mesh.getHalfEdge( mesh.getVertex(from), mesh.getVertex(to) );
            if( edge )
            {
                stream >> edge->getData();
            }
